...
public abstract class GenericServiceImpl<T extends BaseEntity, ID extends Serializable, DTO> 
    ...
    public List<DTO> findAll() {
        try {
            ...
            throw new GenericException("DATABASE_ERROR", 
                "Error al acceder a los datos de " + entityClass.getSimpleName(), ex);
        }
    }
    ...
    public DTO findById(ID id) {
        T entity = repository.findById(id)
                .orElseThrow(() -> new GenericException("NOT_FOUND", 
                    entityClass.getSimpleName() + "  Registro no encontrado con ID: " + id));
    ...
    public DTO save(DTO dto) {
        try {
            ...
            throw new GenericException("DATABASE_ERROR", 
                "Error al guardar " + entityClass.getSimpleName(), ex);
        }
    }
    ...
    public DTO update(ID id, DTO dto) {
        try {
            T existingEntity = repository.findById(id)
                    .orElseThrow(() -> new GenericException("NOT_FOUND", 
                        entityClass.getSimpleName() + " Registro no encontrado para actualizar. ID: " + id));
            ...
            throw new GenericException("DATABASE_ERROR", 
                "Error al actualizar " + entityClass.getSimpleName(), ex);
        }
    }
    ...
    public void deleteById(ID id) {
        if (!repository.existsById(id)) {
            throw new GenericException("NOT_FOUND", 
                entityClass.getSimpleName() + " Registro no encontrado para eliminar. ID: " + id);
        }
        ...
        try {
            ...
            throw new GenericException("DATABASE_ERROR", 
                "Error al eliminar " + entityClass.getSimpleName(), ex);
        }
    }
    ...
    protected void validateDTO(DTO dto) {
        ...
        if (!violations.isEmpty()) {
            ...
            throw new GenericException("VALIDATION_ERROR", 
                "Validación fallida en " + dtoClass.getSimpleName() + ": " + messages);
        }
    }
    ...
    protected void validateEntity(T entity) {
        ...
        if (!violations.isEmpty()) {
            String messages = violations.stream()
                    ...
                    .collect(Collectors.joining("; "));
            throw new GenericException("VALIDATION_ERROR", 
                "Validación fallida en " + entityClass.getSimpleName() + ": " + messages);
        }
    }

    protected void validateDeletion(ID id) {
        if (repository.existsById(id)) {
            throw new GenericException("DELETE_FAILED", 
                "Falló la eliminación de " + entityClass.getSimpleName() + " con ID: " + id);
        }
    }
    ...
    public Page<DTO> findAllPaginated(Pageable pageable) {
        try {
            ...
            throw new GenericException("DATABASE_ERROR", 
                "Error en paginación de " + entityClass.getSimpleName(), ex);
        }
    }
}